...
var ipCache = require('./utils/ip-cache');
var MAX_HISTORY = 40;
...
Node.prototype.setGeo = function(ip)
{
	...
	this.info.ip = ip;
	const self = this;
	if (ipCache[ip] && Date.now() - ipCache[ip].timestamp < 3600000) {
    self.geo = ipCache.geo;
	} else {
    request('https://ipapi.co/' + ip + '/json/', function (error, res, body) {
      try {
        self.geo = JSON.parse(body);
        ipCache[ip] = {
          timestamp: Date.now(),
					geo: self.geo
				}
